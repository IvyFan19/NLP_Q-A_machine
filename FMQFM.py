...
Author: Eduard Kegulskiy, Xinwei Fan
...
class FactoidQueryParser:
        ...
        self._stop_words.update(
            ('Where', 'Who', 'Whose', 'What', 'Why', 'How', 'and', 'I', 'A', 'And', 'So', 'arnt', 'This',
             'When', 'It', 'many', 'Many', 'so', 'cant', 'whom'
                                                         'Yes', 'yes', 'No', 'no', 'These', 'these', 'is', 'are', 'Do',
             "Are", "About", "For", "Is", "\""))
    ...
    def log(self, text):
        # module 2, first print:  
        # Transforming question terms into likely answer form: first pyramid build in Egypt
    ...
    def _replace_factoid_question_words(self):
        ...
        # Added by Ivy
        self._parsed_query = self._parsed_query.replace("Which year was", "")
        self._parsed_query = self._parsed_query.replace("which year was", "")
    ...
    def _post_process_queries(self):
        ...
        if self._full_query is None:
            ...
            search_queries.append(
                (self._full_query, FactoidQueryParser.POS_QUERY, FactoidQueryParser.FULL_PLUS_QUERY_RANK))
        else:
            search_queries_staging.append(self._full_query)
            search_queries.append(
                (self._full_query, FactoidQueryParser.FULL_PLUS_QUERY, FactoidQueryParser.FULL_PLUS_QUERY_RANK))

        index = 0
        num_strategies = len(pos_strategies)
        while (is_single_gram_query(self._pos_query) or self._pos_query == self._full_query or above_gram_limit(
                self._pos_query)) \
                and num_strategies > 0:
